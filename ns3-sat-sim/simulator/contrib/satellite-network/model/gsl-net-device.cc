...
GSLNetDevice::TransmitStart (Ptr<Packet> p, const Address dest)
{
  ...
  //----OTRA FORMA DE INTENTAR LOGUAR SALIDA PAQUETES---- 
  NS_LOG_INFO("Nodo " << this->GetNode()->GetId() << " está transmitiendo un paquete con UID: " << p->GetUid());

  // Filtrar solo los eventos para el nodo A (GID 1174)
  //static uint32_t lastTxUid = 0;  // Variable estática para almacenar el último UID transmitido

    //if (this->GetNode()->GetId() == 1174 && p->GetUid() != lastTxUid) {
        //std::cout << "Nodo A (1174) está transmitiendo un paquete con UID: " << p->GetUid() << std::endl;
        //lastTxUid = p->GetUid();  // Actualizar el último UID transmitido
    //}  
  //--------
...
GSLNetDevice::Receive (Ptr<Packet> packet)
{
  ...
  NS_LOG_INFO("Nodo " << this->GetNode()->GetId() << " ha recibido un paquete con UID: " << packet->GetUid());
   //----OTRA FORMA DE INTENTAR LOGUEAR ENTRADA DE PAQUETES----
   // Filtrar para satélites de primer salto específicos 
   //static uint32_t lastRxUid = 0;  // Variable estática para almacenar el último UID recibido

   //std::set<int> satelitesPrimerSalto = {236, 268, 235, 269};
   //if (satelitesPrimerSalto.find(this->GetNode()->GetId()) != satelitesPrimerSalto.end() && packet->GetUid() != lastRxUid) {
        //std::cout << "Satélite " << this->GetNode()->GetId() << " ha recibido un paquete con UID: " << packet->GetUid() << std::endl;
        //lastRxUid = packet->GetUid();  // Actualizar el último UID recibido
    //}    
