...
void TcpFlowSendApplication::SendData(void) {
    NS_LOG_FUNCTION(this<<" MaxBytes "<<m_maxBytes<<" m_sendSize "<<m_sendSize);
    while (m_maxBytes == 0 || m_totBytes < m_maxBytes) { // Time to send more
        ...
        NS_LOG_LOGIC("sending packet at " << Simulator::Now() << " tam bloque se pasa a socket "<<toSend<<" quedan "<<m_maxBytes-m_totBytes<<" a traves del socket "<<m_socket);
