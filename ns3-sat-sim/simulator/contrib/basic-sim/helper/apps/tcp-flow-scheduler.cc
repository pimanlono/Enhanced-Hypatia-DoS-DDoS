...
NS_LOG_COMPONENT_DEFINE ("TcpFlowScheduler");
...
void TcpFlowScheduler::WriteResults() {
    ...
    // Files for legitimate TCP flows
    std::cout << "  > Opening TCP flow log files:" << std::endl;
    FILE* tcp_csv = fopen(m_flows_csv_filename.c_str(), "w+");
    std::cout << "    >> Opened: " << m_flows_csv_filename << std::endl;
    FILE* tcp_txt = fopen(m_flows_txt_filename.c_str(), "w+");
    ...
    // TCP flows header
    fprintf(
            tcp_txt, "%-16s%-10s%-10s%-16s%-18s%-18s%-16s%-16s%-13s%-16s%-14s%s\n",
    ...
    for (TcpFlowScheduleEntry& entry : m_schedule) {
        ...
        // For legitimate TCP flows, we get the corresponding app
        ...
        fprintf(
                tcp_csv, "%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 ",%s,%s\n",
                    entry.GetTcpFlowId(), entry.GetFromNodeId(), entry.GetToNodeId(), entry.GetSizeByte(), entry.GetStartTimeNs(),
                    entry.GetStartTimeNs() + fct_ns, fct_ns, sent_byte, finished_state.c_str(), metadata.c_str()
            );
        ...
        fprintf(
                tcp_txt, "%-16" PRId64 "%-10" PRId64 "%-10" PRId64 "%-16s%-18" PRId64 "%-18" PRId64 "%-16s%-16s%-13s%-16s%-14s%s\n",
                    ...
                    finished_state.c_str(), metadata.c_str()
            );
