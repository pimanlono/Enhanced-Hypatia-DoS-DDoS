#include "attack-flow-schedule-reader.h"

namespace ns3 {

AttackFlowScheduleEntry::AttackFlowScheduleEntry(
        int64_t attack_flow_id,
    ...
    m_attack_flow_id = attack_flow_id;
...
int64_t AttackFlowScheduleEntry::GetAttackFlowId() {
    return m_attack_flow_id;
}

int64_t AttackFlowScheduleEntry::GetFromNodeId() {
    return m_from_node_id;
}

int64_t AttackFlowScheduleEntry::GetToNodeId() {
    return m_to_node_id;
}

int64_t AttackFlowScheduleEntry::GetSizeByte() {
    return m_size_byte;
}

int64_t AttackFlowScheduleEntry::GetStartTimeNs() {
    return m_start_time_ns;
}

std::string AttackFlowScheduleEntry::GetAdditionalParameters() {
    return m_additional_parameters;
}

std::string AttackFlowScheduleEntry::GetMetadata() {
    return m_metadata;
}
...
*/
std::vector<AttackFlowScheduleEntry> read_attack_flow_schedule(const std::string& filename, Ptr<Topology> topology, const int64_t simulation_end_time_ns) {

    // Schedule to put in the data
    std::vector<AttackFlowScheduleEntry> schedule;
    ...
    if (schedule_file) {
        ...
        while (getline(schedule_file, line)) {
            ...
            // Fill entry
            int64_t attack_flow_id = parse_positive_int64(comma_split[0]);
            if (attack_flow_id != (int64_t) line_counter) {
                throw std::invalid_argument(format_string("ATTACK flow ID is not ascending by one each line (violation: %" PRId64 ")\n", attack_flow_id));
            }
            ...
            if (prev_start_time_ns > start_time_ns) {
                throw std::invalid_argument(format_string("Start time is not weakly ascending (on line with ATTACK flow ID: %" PRId64 ", violation: %" PRId64 ")\n", attack_flow_id, start_time_ns));
            }
            prev_start_time_ns = start_time_ns;

            // Check node IDs
            if (from_node_id == to_node_id) {
                throw std::invalid_argument(format_string("ATTACK flow to itself at node ID: %" PRId64 ".", to_node_id));
            }
            ...
            if (start_time_ns >= simulation_end_time_ns) {
                throw std::invalid_argument(format_string(
                        "ATTACK flow %" PRId64 " has invalid start time %" PRId64 " >= %" PRId64 ".",
                        attack_flow_id, start_time_ns, simulation_end_time_ns
                ));
            }
            // Put into schedule
            schedule.push_back(AttackFlowScheduleEntry(attack_flow_id, from_node_id, to_node_id, size_byte, start_time_ns, additional_parameters, metadata));
