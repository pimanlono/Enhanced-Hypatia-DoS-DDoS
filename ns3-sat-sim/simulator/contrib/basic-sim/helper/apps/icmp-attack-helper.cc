...
IcmpAttackHelper::IcmpAttackHelper(std::string socketType, 
                               Address address,
                               int64_t flowId,
                               bool enableLogging,
                               std::string baseLogsDir,
			       int64_t icmpInterval,
                               int64_t attackDuration) //new
                               
{
    m_factory.SetTypeId("ns3::IcmpAttackApplication");
    ...
    m_factory.Set("IcmpInterval", TimeValue(NanoSeconds(icmpInterval)));
    // Set EndAttackTime 
    m_factory.Set("AttackDuration", TimeValue(NanoSeconds(attackDuration)));
}

ApplicationContainer
IcmpAttackHelper::Install(Ptr<Node> node) const
{
    return ApplicationContainer(InstallPriv(node));
}

Ptr<Application>
IcmpAttackHelper::InstallPriv(Ptr<Node> node) const
{
