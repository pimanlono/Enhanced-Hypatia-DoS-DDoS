...
//PILAR
  class PendingCount : public Object
  {
  public:
    static TypeId GetTypeId (void)
    {
      static TypeId tid = TypeId ("ns3::TcpSocketBase::PendingCount")
        .SetParent<Object> ()
        .AddConstructor<PendingCount> ();
      return tid;
    }
    PendingCount () : m_count (0) {}

    void Increment () { ++m_count; }
    void Decrement () { if (m_count > 0) --m_count; }
    uint32_t Get () const { return m_count; }
...
public:
   ...
   //Pilar
   //static uint32_t m_PendingConnections;
   //static uint32_t m_maxPendingConnections;
...
protected:
  ...
  //PILAR
  Ptr<PendingCount> m_PendingConnections;
  uint32_t m_maxPendingConnections=128;
