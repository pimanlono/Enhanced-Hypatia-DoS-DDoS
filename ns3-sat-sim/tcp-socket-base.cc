...
//Pilar
//uint32_t TcpSocketBase::m_PendingConnections=0;
//uint32_t TcpSocketBase::m_maxPendingConnections=128;
  ...
  : TcpSocket ()
{
  ...
  //PILAR
  m_PendingConnections = CreateObject<PendingCount>();
    ...
    m_ecnCWRSeq (sock.m_ecnCWRSeq)
{
  ...
  //PILAR
  m_PendingConnections = sock.m_PendingConnections;
  m_maxPendingConnections = sock.m_maxPendingConnections;
                              ...
                              const Address& fromAddress, const Address& toAddress)
{
  ...
  //PILAR
  std::cout<<std::endl<<this<<" Server: Nuevo SYN recibido at "<<Simulator::Now().GetSeconds()<<" from "<<InetSocketAddress::ConvertFrom(fromAddress).GetIpv4()<<" port "<<tcpHeader.GetSourcePort()<<std::endl;
  if (m_PendingConnections->Get()<m_maxPendingConnections)
  {
	  m_PendingConnections->Increment();
	  std::cout<<this<<" SYN recibido y aun hay espacio, incremento m_PendingConnections a "<<m_PendingConnections->Get()<<std::endl<<std::endl;
  }
  else {
	  std::cout<<this<<" SYN recibido y no hay espacio en m_PendingConnections "<<m_PendingConnections->Get()<<std::endl<<std::endl;
	  return;
  }
  ...
  if (!NotifyConnectionRequest (fromAddress))
    {
      std::cout<<"PILAR: NotifyConnectionRequest da 0"<<std::endl;
      return;
  }
...
TcpSocketBase::ProcessSynSent (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  if (tcpflags == 0)
    ...
    { // Handshake completed
      ...
      //PILAR
      std::cout<<this<<" Client: Conexion establecida. Port: "<<tcpHeader.GetDestinationPort()<<" "<<Simulator::Now().GetSeconds()<<std::endl;
                               ...
                               const Address& fromAddress, const Address& toAddress)
{
  ...
  if (tcpflags == 0
    ...
    { // If it is bare data, accept it and move to ESTABLISHED state. This is
      ...
      //PILAR
      std::cout<<this<<" Server: Conexion establecida con "<<InetSocketAddress::ConvertFrom(fromAddress).GetIpv4()<<" port "<<tcpHeader.GetSourcePort()<<" decremento m_PendingConnections "<<Simulator::Now().GetSeconds()<<std::endl;
      m_PendingConnections->Decrement();
      std::cout<<this<<" ------------------ m_PendingConnections "<<m_PendingConnections->Get()<<std::endl<<std::endl;
  ...
  if (hasSyn)
    {
      ...
      if (m_synCount == 0)
        { // No more connection retries, give up
          NS_LOG_LOGIC ("Connection failed "<<Simulator::Now().GetSeconds());
	  //PILAR
	  std::cout<<this<<" Connection failed con "<<m_endPoint->GetPeerAddress()<<" port "<<m_endPoint->GetPeerPort()<< " decremento m_PendingConnections "<<Simulator::Now().GetSeconds()<<std::endl;
	  m_PendingConnections->Decrement();
	  std::cout<<this<<" ------------------ m_PendingConnections "<<m_PendingConnections->Get()<<std::endl<<std::endl;
	  ...
	  std::cout<<this<<" Soy "<<m_endPoint->GetLocalAddress()<<" port "<<m_endPoint->GetLocalPort()<<" Envio SYN a "<<m_endPoint->GetPeerAddress()<<" port "<<m_endPoint->GetPeerPort()<<" Valor de m_synCount "<<m_synCount<<" "<<Simulator::Now().GetSeconds()<<std::endl<<std::endl;
